...
courseGrades <- read.csv("CourseGrades.csv")
colnames(courseGrades)[1] <- "HASH_ID"
courseGrades$Grade = as.numeric(as.character(courseGrades$Grade))

midterm <- 
  inner_join(
    read.csv("Midterm1_Fall_2020.csv"),
    read.csv("Midterm2_Fall_2020.csv"), 
    by = "HASH_ID") %>% 
...
```{r}
grade_wide = select(grade,HashedId,variable,rank) %>% spread(key = variable, value = rank)
